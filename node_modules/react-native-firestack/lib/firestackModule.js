...
export class FirestackModule {
  ...
  listen(cb) {
    ...
    const _itemChanged = (snapshot, prevKey) => {
      ...
      let list = state.items || {};
      list[existingItem._key] = existingItem;
  ...
  _getState() {
    ...
    if(isUndefined(state[this._refName])) {
      // injectRed
      injectReducer(store, { key: this._refName, reducer: this.reducer })
    }
